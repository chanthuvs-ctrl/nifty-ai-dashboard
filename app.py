...
def get_market_data():
    ...
    return {
        ...
        "daily_brokerage": round(sum(20.0 for p in state.strategy_positions.values() if p and p.get("status") == "OPEN") + sum(t.get("brokerage", 20.0) for t in journal.trades), 2),
        ...
        "today_trades": sum(1 for p in state.strategy_positions.values() if p and p.get("status") == "OPEN") + sum(1 for t in journal.trades if t.get("date") == get_ist_date_str()),
